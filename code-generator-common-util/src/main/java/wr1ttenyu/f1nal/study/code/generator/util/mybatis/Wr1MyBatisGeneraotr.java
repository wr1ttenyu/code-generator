 ...
 */
public class Wr1MyBatisGeneraotr {
     ...
     */
    public Wr1MyBatisGeneraotr(Configuration configuration, ShellCallback shellCallback,
     ...
     */
    public byte[] generate(ProgressCallback callback) throws Exception {
         return generate(callback, null, null, true);
    }
     ...
     *             if the method is canceled through the ProgressCallback
     */
    public byte[] generate(ProgressCallback callback, Set<String> contextIds,
                         Set<String> fullyQualifiedTableNames) throws Exception {
        return generate(callback, contextIds, fullyQualifiedTableNames, true);
    }
     ...
     */
    public byte[] generate(ProgressCallback callback, Set<String> contextIds,
                         Set<String> fullyQualifiedTableNames, boolean writeFiles) throws Exception {
        ...
        List<FileEntryModel> fileEntryModels = new ArrayList<>();
        for (GeneratedXmlFile gxf : generatedXmlFiles) {
            fileEntryModels.add(new FileEntryModel(gxf.getFormattedContent().getBytes("UTF-8"),
                gxf.getTargetPackage(), gxf.getFileName()));
        }

        for (GeneratedJavaFile gjf : generatedJavaFiles) {
            fileEntryModels.add(new FileEntryModel(gjf.getFormattedContent().getBytes("UTF-8"),
                    gjf.getTargetPackage(), gjf.getFileName()));
        }

        byte[] zip = ZipCompress.zip(fileEntryModels);
